...
#media_format = 'https://airnet.org.au/omnystudio/3pbs/{slug}/{timestamp}/aac_mid.m4a'  # timestamp = yyyymmddhhmm

#media_format = 'https://airnet.org.au/omnystudio/3pbs/{slug}/{timestamp'  # this is all that's needed
media_format = 'https://airnet.org.au/omnystudio/3pbs/{slug}/{timestamp}/{slug}{timestamp}.mp3'
...
def all_shows():
    ...
    for show in shows:
        if show['slug'] and not show['archived']:
            shows_data.append((show['name'], 
                              "/"+show['slug']+".xml", 
                              show['gridDescription'],
                              show['broadcasters']))
    print(shows_data)
...
def pbs_show(slug):
    ...
    for episode in reversed(episodes):
        ...
        feed_entry.enclosure(media_url, 0, 'audio/mpeg')
        try:
